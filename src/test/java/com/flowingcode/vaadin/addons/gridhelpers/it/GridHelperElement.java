...
public class GridHelperElement extends MyGridElement {
   ...
   *
   * <p>
   * Toggling the visibility of a column makes the grid re-render the column tree, which invalidates
   * the row and its slots. The lookup is retried while that happens, so that a stale element or a
   * cell whose slot is not rendered yet is not reported as an absent child.
   */
